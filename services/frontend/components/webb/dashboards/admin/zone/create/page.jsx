...
export default function CreateNewZone({ cityId }) {
  ...
  async function handleAction(formData) {
    try {
      ...
      if (type === 'polygon') {
        ...
        if (!areaText) throw new Error('Area is required for polygons.');
        ...
        const area = areaText
          ...
          .map(line => {
            ...
            if (Number.isNaN(lat) || Number.isNaN(lng)) {
              throw new Error('Invalid polygon coordinate format');
            }
            return [lat, lng];
          });
  ...
  return (
    <div className="w-md mx-auto border border-detail-yellow rounded-md p-6 shadow-xl bg-white">
      <h2 className="text-3xl text-black mb-6 text-center">Create Zone</h2>
